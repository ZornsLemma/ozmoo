...
; - just checking things, with 128K SWR, uncapped .ram_pages=$0284, nonstored_pages=$78, vmap_max_entries=$fa so it all looks very similar - since uncapped .ram_pages is larger than the game, this makes sense
; - with 112K SWR, vmap_max_entries=$ff, vmem_blocks_stolen_in_first_bank=$07, vmap_meaningful_entries=$fa, uncapped .ram_pages=$0244, nonstored_pages=$46 - after the real dynamic memory of $3c pages is allocated, the game has a further $230 pages of swappable data. Since we only have $208 pages after allocating $3c for real dynmem, it is superficially odd that we've chosen to increase nonstored_pages. OK, the point is that we have $26c-$3c=$230 pages of game vmem and we have $208 pages of RAM for vmem cache, *but* vmap can't have more than 255 entries so we can't actually address all that RAM as vmem cache. So if we use 255 vmap entries, we can address $1fe pages of RAM as vmem cache, leaving $a pages which would allow us to increase nonstored_pages from $3c to $3c+$a=$46, which is what we see. vmap_meaningful_entries is $ff-($a/2)=$fa because our initial vmap doesn't have useful stuff for those vmem pages as we're loading more data for our extended dynmem, but the full $ff entries of vmap can be used for caching data as the game runs.
