...
; - mulling this over, you'd probably want to ensure the two levels of cache didn't have any of the same data. (Bear in mind sideways RAM could well be smaller than the second processor's own cache.) So the OSWORD would probably be more of a "here's my block of data, I'm evicting this, you put it in your cache (evicting something of your choosing) and then let me have new block of data X, either from your cache - in which case you mark that slot of your cache as free - or just by loading from disc directly into my RAM bypassing your cache" - probably lots of holes there but that's the gist
; - actually it's probably better and easier if the OSWORD just says "I am about to discard this block of data which is block N, save it if you like. And do you have block M? If you do, please put it in my memory and return TRUE, else returen FALSE" and the 2P code continues to do the read from disc. This minimises differences between with and without this code, no need to make the host OSWORD code be capable of opening and closing files on ADFS when disc might be changed etc.
