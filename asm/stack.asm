...
z_ins_catch
	...
	ldx stack_ptr
	lda stack_ptr + 1
        ; SFTODONOW: This (and code in z_ins_throw) is new in 14.x, it's probably fixing a bug and necessary, but be good to check to see the commit which added it and why and if it's necessary and correct for Acorn. - OK, I *suspect* this fixes a long-term bug where a save game with a "catch" in effect would not work correctly if restored on another machine/Ozmoo build with a different stack_start and a throw subsequently executed - be good to review this and check, but that seems plausible and I guess this case is maybe rare and also unlikely to have been tested (I honestly don't know how much users save and restore at all, let alone with e.g. PAGE having changed, and it's not as if there are billions of users) - this *may* (test it and document it so I mention it in stardot post) mean saves are incompatible between older versions and this one, although I guess it may be hard to test reliably, but have a quick try
	sec
	sbc #>stack_start
	jmp z_store_result
