...
# SFTODO: Should we allow colour names like "black" and "red" as well as numeric colour numbers?
...
# SFTODONOW: When undo is supported, if the "game wants to use the undo opcodes" bit in the header is not set for z5+ games, we should warn but otherwise ignore any "build with undo support" option the user has specified. This may never happen - I haven't examined any z5 games - but it should be near-trivial to implement.

# SFTODONOW: Just thinking out loud...
# I am leaning towards allocating space for the undo buffer by shrinking vmap_max_entries (before we do the initial load), and accessing the undo buffer via the regular virtual memory subsystem (or rather, the lower layer of it which converts a vmap index into a page in of whatever memory is required and generation of a 16-bit zp,y access). The discardable init code can check a (user configurable) number of vmem blocks will be leftover before allocating the undo buffer and just disable undo if there wouldn't be enough left. (Wrinkle: ideally we want the loader to know if undo will be supported, so it can show CTRL-U to undo for <Z5 games on the loader screen. Not sure if this is easy or not.)
