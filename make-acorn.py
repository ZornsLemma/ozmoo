...
# SFTODO: Could/should we clear the screen on a restart? I actually think it's generally nice to have the end of the old game visible, but when the restart loaded an executable into screen memory it does look a bit ugly to have the corruption left there until it scrolls off the screen. We could obviously only clear the screen if the executable overlaps screen memory, but given the build system doesn't know the actual load address this might require fiddly OS calls to query the file's load address and length.
...
# SFTODONOW: There may be a bug in fast hardware scrolling shown up by Varicella. Run it on a M128 in mode 3, and just type 'r' at the first command prompt. A fragment of some scrolled-off text appears on the bottom line. Not sure what's going on. (I am choosing option 3 for a mono game, if that matters.) This does not seem to happen in mode 6 or mode 0. I suspect the precise layout of text and what gets scrolled when plays a role, rather than this being inherently specific to mode 3. Pressing CTRL-S to toggle software scrolling on before typing 'r' seems to avoid it in mode 3. The text in question does get printed hard up against the right edge of the screen (the "no space at end of line" case) not long before the game proper starts, and it is the last such line. (But doing "i" for inventory produces another such line, and that does *not* appear to trigger this behaviour on a subsequent "r".) And to be clear, "r" is a meaningless command. - OK, this can also be reproduced on both a M128 and BBC B with a 6502 co-pro in mode 3. Setting a breakpoint on OSWRCH, it *does* appear that it's the handling of the LF character (presumably via the fast scroll code) which is generating this stray text, not something inside the main Ozmoo code. I suspect it's got something to do with the fact the mode 3 screen has a tiny bit of mobile dead space in its 16K, but that's just a hunch at this point.
