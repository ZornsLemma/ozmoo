...
# SFTODO: It *might* be better to have a single make.rb which handles Commodore or
# Acorn, but for the moment let me just fork this file.
# SFTODO: At the moment I've haven't tried to prune out every bit of unused/unsupported
# stuff, because some of it might be supported later.
...
def build_interpreter()
	...
	# SFTODO: Most of this is irrelevant, but just possibly we might use a couple of
	# these options to allow the user to specify the default fg/bg for our mono screen.
...
# $start_address must be on a page boundary, otherwise our hack to avoid duplicating the
# preloaded part of the game won't work.
